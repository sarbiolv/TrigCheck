...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   int GetCrossBunch(int bcid, std::vector<std::pair<int, int>> bcid_trains);
...
#ifdef TrigCheck_cxx
TrigCheck::TrigCheck(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   string path = "/media/sergio/8e49a8ae-42cd-45af-8778-543c9c263d83/second_home/data";
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(Form("%s/428770/428770_L1RD0_new.root",path.c_str()));
      if (!f || !f->IsOpen()) {
         f = new TFile(Form("%s/428770/428770_L1RD0_new.root",path.c_str()));
      }
...
Int_t TrigCheck::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t TrigCheck::LoadTree(Long64_t entry)
{
...
void TrigCheck::Init(TTree *tree)
{
...
void TrigCheck::Show(Long64_t entry)
{
...
Int_t TrigCheck::Cut(Long64_t entry)
{
